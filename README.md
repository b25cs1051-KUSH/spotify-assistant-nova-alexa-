...
* **Restricted Standby Grammar (10ms Wake Latency):** The local offline Vosk wake-word recognizer uses a restricted vocabulary grammar (`["alexa", "nova", "innova", "noa", "[unk]"]`). This limits the search dictionary from 100,000+ words to 5, dropping standby wake latency to ~10ms and preventing false triggers entirely (out-of-vocabulary words map to `[unk]` and are ignored).
* **Native Keyboard Media Keys (0ms playback controls):** Bypasses Spotify Web API network requests for basic playback commands (play, pause, next, prev) by sending native keyboard events directly to the Windows OS using C-level `ctypes` (`VK_MEDIA_STOP`, `VK_MEDIA_PLAY_PAUSE`, `VK_MEDIA_NEXT`, `VK_MEDIA_PREV`).
* **Dynamic Voice Activity Detection (VAD):** Employs real-time Root Mean Square (RMS) energy thresholding. When you finish speaking a command, it detects the silence threshold and terminates recording after 600ms, saving up to 3 seconds of dead silence wait time.
